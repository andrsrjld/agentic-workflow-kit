...
1. **Scaffold structure** from `~/.agentic-workflows/templates/`:
   copy `claude/` (commands, agents, hooks, settings snippet), gate `scripts/`,
   and the `/docs` skeletons. Substitute `{{placeholders}}` from answers/detection.
...
5. **Kick off** — run the first unit of work: `/nerve "<first task>"` or
...
3. **Drop generic tooling** — copy gate `scripts/`, `.claude/hooks/` (guardrail +
   nerve), `.claude/agents/`, `.claude/commands/`, and merge `settings.json`
...
5. **Verify additive** — `git diff` must show only `.agentic/`, `.claude/`,
